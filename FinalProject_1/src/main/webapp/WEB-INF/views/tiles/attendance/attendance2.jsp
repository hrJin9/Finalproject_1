			...
			$.ajax({
				...
				success:function(json){ 
					...
					if(json[0].workTime != 0 && json[0].workMin != 0) {
						...
						$("#workTime"+index+"> span").text(json[0].workTime+"시간");
					} else if(json[0].workTime == "" && json[0].workMin == ""){  // 처리 필요 !!!
						// $("#workTime"+index+"> span").text("0시간");
						// $("#workTime"+index+"> span").css("display","none");
					}
					 */
				},
				error: function(request, status, error){
...
<div class="attendance-container">
	...
	<div class="workingweek">
		<table class="fontsize-basic worktable" style="position: relative; z-index: 99;">
			...
			<tr>
				<td class="tue seldate"><span class="date"></span>(화)<span id="datedot2"></span>
					<div class="workingtotalhourByday" id="workTime1"><span>시간</span></div>
				</td>
				<%-- <c:forEach var="i" begin="0" end="47"><td></td></c:forEach> --%>
			</tr>
			<tr>
				<td class="wed seldate"><span class="date"></span>(수)<span id="datedot3"></span>
			...
			<tr>
				<td class="fri seldate"><span class="date"></span>(금)<span id="datedot5"></span>
		  	...
		  	<form id="attendance_frm">
			  	<div>
			  		<div class="todayworkStatus-container">
				  		...
				  		<div class="atnotice">(휴게시간 미포함)</div>
				  		<div class="line"></div>
			  		</div>
			  		<!-- 근무 1개  -->
