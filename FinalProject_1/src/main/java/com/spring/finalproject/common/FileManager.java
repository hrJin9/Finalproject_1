...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		newFileName += fileExt;
		// newFileName은 예를들면, 202210130810329812309123121.png아 같이 됨
		...
		if(!dir.exists()) {	// 파일을 저장할 경로인 폴더가 실제로 존재하지 않는다면
			// 파일을 저장할 경로인 폴더를 생성
	...
	public boolean dofileDownload(String fileName, String orgFilename, String path, HttpServletResponse response) {
		...
		if("".equals(orgFilename) || orgFilename == null) {
			orgFilename = fileName; //혹시 없다면 기존 filename으로 주겠다는 의미
		}
		
		try {
			orgFilename  = new String(orgFilename.getBytes("UTF-8"), "8859_1"); //한글깨짐 방지
			// orgFilename.getBytes("UTF-8") 은 UTF-8 형태로 되어진 문자열 orgFilename을  byte 형태로 변경한 후 
			// byte 형태로 되어진 거을 표준인 ISO-Latin1(혹은 Latin1 또는 8859_1) 형태로 인코딩한 문자열로 만든다.
		...
		try {
			...
			// 다운로드할 파일명(pathname)을 가지고 File객체를 생성
